...
class KaryawanController extends Controller
{
    ...
    {
        try {
            // ✅ OPTIMIZED: Select only needed columns + active_project
            ...
            ->with([
                'divisi:id,nama', 
                'jabatan:id,nama',
                // ✅ NEW: Load active project
                'activeProject.project:id,nama'
            ]);

            // Search - optimized with index hints
            ...
            if ($request->filled('project_id') && $request->project_id !== 'all') {
                if ($request->project_id === 'unassigned') {
                    // Filter karyawan yang belum di-assign ke project manapun
                    $karyawans->whereDoesntHave('activeProject');
                } else {
                    // Filter karyawan yang ada di project tertentu
